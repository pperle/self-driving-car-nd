...
void UKF::Initialization(const MeasurementPackage &meas_package) {
  double px;
  double py;
  double vx;
  double vy;
  ...
  if (meas_package.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      cout << "Initialization Radar" << endl;
      ...
      cout << "Initialization Laser" << endl;
  ...
  // Handle small px, py
  // px = fabs(px) < p_x_min ? p_x_min_ : px;
  // py = fabs(py) < p_y_min ? p_y_min_ : py;
...
void UKF::AugmentedSigmaPoints() {
  ...
  //print result
  cout << "Xsig_aug = " << endl << Xsig_aug_ << endl;
}
...
void UKF::PredictMeasurement(int n_z) {
  ...
  for (int i = 0; i < n_sig_; i++) {  //2n+1 simga points
    ...
    // measurement model
    if (n_z >= 3) {
      Zsig_(0, i) = max(0.0001, sqrt(p_x * p_x + p_y * p_y));
